...
def perform_time_delay_estimation(file_paths, num_input_signals_list, num_output_signals_list):
    ...
    degree = 4  # Degree of the polynomial regression Model
    order = 4 # Order of ARX model
    allresults = []
    allresults2 = []
    for file_path, num_input_signals, num_output_signals in zip(file_paths, num_input_signals_list, num_output_signals_list):
        ...
        for output_col in output_columns:
            ...
            def cross_correlation(input_signals, output_signal, sampling_rate):
                input_signals_array = input_signals.to_numpy()
                output_signal_array = output_signal.to_numpy()
                N = len(input_signals_array[1])
                num_rows = len(input_signals_array)
                cross_corr = np.zeros((num_rows, N))
                for i in range(num_rows):
                    for k in range(N):
                        cross_corr[i, k] = np.sum(input_signals_array[i, k:] * output_signal_array[0:N-k])
                time_delay = np.argmax(cross_corr, axis=1) / sampling_rate
                
                return np.argmin(time_delay)
            ...
            def lstm_time_delay(input_signals, output_signal, epochs=100, batch_size=batchSize):
                ...
                time_delay = cross_correlation(pd.DataFrame(predicted_output), output_signal, sampling_rate)
            ...
            estimated_time_delay_CrossCorr = cross_correlation(input_signals, output_signal, sampling_rate)
        ...
        df.to_csv('Code1.csv', index=False)
        df2.to_csv('Code2.csv', index=False)
